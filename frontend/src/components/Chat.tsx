...
export default function Chat({ inLobby = false }: { inLobby?: boolean }) {
  ...
  const { messages, me } = useGameStore();
    ...
    <div className="flex flex-col h-full bg-neutral-900">
      {!inLobby && (
        <div className="p-3 bg-neutral-800 border-b border-white/5 shadow-sm z-10">
          <h3 className="font-bold text-sm text-neutral-300 uppercase tracking-widest text-center">Chat & Guesses</h3>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 font-medium text-sm">
        {messages.map((msg, i) => {
          ...
          if (msg.isSystem) {
             return (
                 <div key={i} className="text-center text-xs p-1.5 my-2 rounded-lg bg-white/5 text-white/50 border border-white/10 font-bold mx-4 shadow-sm">
                     {msg.message}
                 </div>
             )
          }
          ...
          if (msg.isCorrectGuess) {
             return (
                 <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-green-500/20 text-green-400 border border-green-500/30 font-bold max-w-[90%] w-fit shadow-md">
